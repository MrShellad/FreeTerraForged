...
public class PresetNoiseRouterData {
    ...
    public static void bootstrap(Preset preset, BootstrapContext<DensityFunction> ctx) {
        ...
        ctx.register(NoiseRouterData.CONTINENTS, FTFDensityFunctions.cell(CellSampler.Field.CONTINENT));
        ctx.register(NoiseRouterData.EROSION, FTFDensityFunctions.cell(CellSampler.Field.EROSION));
        ctx.register(NoiseRouterData.RIDGES, FTFDensityFunctions.cell(CellSampler.Field.WEIRDNESS));

        DensityFunction height = NoiseRouterData.registerAndWrap(ctx, HEIGHT, FTFDensityFunctions.cell(CellSampler.Field.HEIGHT));
        DensityFunction offset = NoiseRouterData.registerAndWrap(ctx, NoiseRouterData.OFFSET, DensityFunctions.add(DensityFunctions.constant(NoiseRouterData.GLOBAL_OFFSET - 0.5F), DensityFunctions.mul(FTFDensityFunctions.clampToNearestUnit(height, properties.terrainScaler()), DensityFunctions.constant(2.0D))));
        ...
        ctx.register(GRADIENT, FTFDensityFunctions.cell(CellSampler.Field.GRADIENT));
        ctx.register(HEIGHT_EROSION, FTFDensityFunctions.cell(CellSampler.Field.HEIGHT_EROSION));
        ctx.register(SEDIMENT, FTFDensityFunctions.cell(CellSampler.Field.SEDIMENT));
    }
    ...
    protected static NoiseRouter overworld(Preset preset, HolderGetter<DensityFunction> densityFunctions, HolderGetter<NormalNoise.NoiseParameters> noiseParams, HolderGetter<Noise> noises) {
        ...
        DensityFunction temperature = FTFDensityFunctions.cell(CellSampler.Field.TEMPERATURE);
        DensityFunction vegetation = FTFDensityFunctions.cell(CellSampler.Field.MOISTURE);
...
//        DensityFunction finalDensity = DensityFunctions.min(NoiseRouterData.postProcess(slideOverworld(slopedCheeseCaves, -worldDepth)), NoiseRouterData.getFunction(densityFunctions, NoiseRouterData.NOODLE));
    ...
    private static DensityFunction blendCoastToInland(
        ...
        DensityFunction alpha = DensityFunctions.mul(
            DensityFunctions.add(
                FTFDensityFunctions.cell(CellSampler.Field.CONTINENT_EDGE),
    ...
    private static ResourceKey<DensityFunction> createKey(String string) {
        return ResourceKey.create(Registries.DENSITY_FUNCTION, FTFCommon.location(string));
    }
