...
public abstract class PresetEditorPage extends BisectedPage<PresetConfigScreen, AbstractWidget, AbstractWidget> {
	...
	private void createControls() {
		...
		this.zoom2D = PresetWidgets.createIntSlider((int) Math.round(initZoom2D), minZoom, maxZoom, FTFTranslationKeys.GUI_SLIDER_ZOOM, (slider, value) -> {
		...
		this.zoom3D = PresetWidgets.createIntSlider((int) Math.round(initZoom3D), minZoom, maxZoom, FTFTranslationKeys.GUI_SLIDER_ZOOM, (slider, value) -> {
		...
		this.renderMode2D = PresetWidgets.createCycle(ImmutableList.copyOf(RenderMode.values()), this.renderMode2D != null ? this.renderMode2D.getValue() : staticMode2D, FTFTranslationKeys.GUI_BUTTON_RENDER_MODE, (button, value) -> {
			staticMode2D = value;
			if (this.preview2D != null) this.preview2D.refreshRenderMode(value);
		}, RenderMode::displayName);

		this.renderMode3D = PresetWidgets.createCycle(ImmutableList.copyOf(RenderMode.values()), this.renderMode3D != null ? this.renderMode3D.getValue() : staticMode3D, FTFTranslationKeys.GUI_BUTTON_RENDER_MODE, (button, value) -> {
			staticMode3D = value;
		...
		this.seedEdit = new EditBox(Minecraft.getInstance().font, 0, 0, 0, 20, Component.translatable(FTFTranslationKeys.GUI_BUTTON_SEED)) {
			@Override
		...
		this.seedEdit.setHint(Component.translatable(FTFTranslationKeys.GUI_BUTTON_SEED));
		...
		this.seedRandomize = Button.builder(Component.literal("🎲"), (button) -> {
				...
				.tooltip(Tooltip.create(Component.translatable(FTFTranslationKeys.GUI_BUTTON_RANDOMIZE_SEED)))
				.bounds(0, 0, 20, 20)
				.build();
	}
