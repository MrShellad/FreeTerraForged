package etcodehome.freeterraforged.world.worldgen.feature.placement;
...
public final class SurfaceFeatureRescue {
	...
	private static final class Run {
		...
		private final SurfacePlacementClassifier.SurfacePipeline pipeline;
		private final UndergroundFeatureEnclosure.Guard enclosure;
		private final Map<ColumnBand, long[]> surfaceCache = new HashMap<>();
		private final Map<DynamicHeightRangePlacement.HeightBand, BandBudget> bandBudgets = new HashMap<>();
		...
		private Run(
			...
			SurfacePlacementClassifier.SurfacePipeline pipeline,
		...
		private Optional<BlockPos> rescue(BlockPos originalOrigin) {
			...
			DynamicHeightRangePlacement.HeightBand band = DynamicHeightRangePlacement.bandContaining(minY, maxY, originY);
			if (!this.bandBudgets
